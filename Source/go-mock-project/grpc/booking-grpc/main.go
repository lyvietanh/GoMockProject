...
import (
	...
	booking_grpc_handlers "anhlv11/go-mock-project/grpc/booking-grpc/handlers"
...
func main() {
	config, err := util.LoadGrpcConfig("./grpc/booking-grpc/")
	if err != nil {
	...
	grpcHandler, err := booking_grpc_handlers.NewBookingGrpcHandler(config, store)
	if err != nil {
		panic(err)
	}

	reflection.Register(grpcServer)
	pb.RegisterBookingGrpcServer(grpcServer, grpcHandler)
