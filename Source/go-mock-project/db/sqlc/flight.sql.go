...
RETURNING id, origin_airport_code, destination_airport_code, departure_date_time, flight_number, booking_class, flight_duration, enabled, seat_available, currency_code, price, create_date, modify_date
`
...
SELECT id, origin_airport_code, destination_airport_code, departure_date_time, flight_number, booking_class, flight_duration, enabled, seat_available, currency_code, price, create_date, modify_date FROM flights
WHERE 
    enabled='1'
...
func (q *Queries) UpdateFlightSeatAvailable(ctx context.Context, arg UpdateFlightSeatAvailableParams) (Flight, error) {
	row := q.db.QueryRowContext(ctx, updateFlightSeatAvailable, arg.ID, arg.SeatAvailable, arg.ModifyDate)
	var i Flight
	err := row.Scan(
		&i.ID,
		&i.OriginAirportCode,
